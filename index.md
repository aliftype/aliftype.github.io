---
title: حروف ألف
subtitle: مسبك حروف عربي
template: default
lang: ar
direction: rtl
redirect_from: /arabic
---

قشيب
----
[{{ picture('assets/images/qashib', 'قشيب') }}](/qashib)
{:.sample}

[اعرف أكثر...](/qashib){:.more}
{:.links}

أسطرلاب
-------
[{{ picture('assets/images/astrolabe', 'أسطرلاب') }}](/astrolabe)
{:.sample}

[اعرف أكثر...](/astrolabe){:.more}
{:.links}

رَقّ
----
[{{ picture('assets/images/raqq', 'رَقّ') }}](/raqq)
{:.sample}

[اعرف أكثر...](/raqq){:.more}
{:.links}

أنت
---
[{{ picture('assets/images/anti', 'أنت') }}](https://github.com/aliftype/anti/releases/latest)
{:.sample}

[نزّل الخط...](https://github.com/aliftype/anti/releases/latest){:.more}
[جت‌هب](https://github.com/aliftype/anti)
{:.links}

قاهري
-----
[{{ picture('assets/images/qahiri', 'قاهري') }}](/qahiri)
{:.sample}

[اعرف أكثر...](/qahiri){:.more}
[خطوط جوجل](https://fonts.google.com/specimen/Qahiri)
{:.links}

كوفي رنا ⚠️🚧
--------
[{{ picture('assets/images/rana-kufi', 'كوفي رنا') }}](/rana-kufi)
{:.sample}

[اعرف أكثر...](/rana-kufi){:.more}
{:.links}

رقعة عارف
---------
[{{ picture('assets/images/aref-ruqaa', 'رقعة عارف') }}](https://github.com/aliftype/aref-ruqaa/releases/latest)
{:.sample}

[نزّل الخط...](https://github.com/aliftype/aref-ruqaa/releases/latest){:.more}
[جت‌هب](https://github.com/aliftype/aref-ruqaa) |
[خطوط جوجل](https://fonts.google.com/specimen/Aref+Ruqaa)
{:.links}

كوفي ريم
--------
[{{ picture('assets/images/reem-kufi', 'كوفي ريم') }}](https://github.com/aliftype/reem-kufi/releases/latest)
{:.sample}

[نزّل الخط...](https://github.com/aliftype/reem-kufi/releases/latest){:.more}
[جت‌هب](https://github.com/aliftype/reem-kufi) |
[خطوط جوجل](https://fonts.google.com/specimen/Reem+Kufi)
{:.links}

مدى
----
[{{ picture('assets/images/mada', 'مدى') }}](https://github.com/aliftype/mada/releases/latest)
{:.sample}

[نزّل الخط...](https://github.com/aliftype/mada/releases/latest){:.more}
[جت‌هب](https://github.com/aliftype/mada) |
[خطوط جوجل](https://fonts.google.com/specimen/Mada)
{:.links}

أميري
-----
[{{ picture('assets/images/amiri', 'أميري') }}](/amiri)
{:.sample}

[اعرف أكثر...](/amiri){:.more}
[ويكيبيديا](https://ar.wikipedia.org/wiki/الخط_الأميري) |
[خطوط جوجل](https://fonts.google.com/specimen/Amiri)
{:.links}

XITS Math
---------
[{{ picture('assets/images/xits', 'XITS Math') }}](https://github.com/aliftype/xits/releases/latest)
{:.sample}

[نزّل الخط...](https://github.com/aliftype/xits/releases/latest){:.more}
[ويكيبيديا](https://en.wikipedia.org/wiki/XITS_font_project) |
[جت‌هب](https://github.com/aliftype/xits)
{:.links}

<a rel="me" href="https://typo.social/@khaled" style="visibility:
hidden;">Mastodon</a>
